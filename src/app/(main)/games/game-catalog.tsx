  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
        {/* Filter Pills */}
        <div className="flex flex-wrap gap-3 w-full md:w-auto justify-center md:justify-start bg-gray-50 dark:bg-gray-900 p-2 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-inner">
          {filters.map((f) => (
            <button
              key={f.id}
        ...
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#5000ef]/50 focus:border-transparent transition-all shadow-sm text-gray-900 dark:text-white placeholder-gray-400"
          />
        </div>
      </div>
      ...
      {filteredGames.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredGames.map((game) => (
            <div 
              key={game.id} 
              className="group relative flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden transform transition duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-[#5000ef]/20 dark:hover:shadow-[0_0_40px_rgba(0,195,203,0.15)]"
            >
              {/* Badges Absolute */}
              ...
              {game.is_multiplayer && (
                <div className="absolute top-4 right-4 z-20">
                  <span className="flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-full text-xs font-bold shadow-md">
                    <Users className="w-3.5 h-3.5" /> 2-4 P
                  </span>
                </div>
              )}
              ...
              <div className="relative w-full aspect-[3/4] bg-gray-100 dark:bg-gray-800 overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 z-20 transform translate-y-2 group-hover:translate-y-0 transition duration-500">
              ...
              <div className="flex flex-col flex-grow p-6">
                ...
                <Link
                  href="/booking"
                  className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl font-bold transition-all duration-300 bg-gray-50 hover:bg-gray-100 text-gray-900 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white border border-gray-200 dark:border-gray-700 group-hover:bg-gradient-to-r group-hover:from-[#5000ef] group-hover:to-[#00c3cb] group-hover:text-white group-hover:border-transparent group-hover:shadow-lg"
                >
                  <Play className="w-4 h-4 fill-current" /> Pesan Sekarang
                </Link>
              </div>
            </div>
          ))}
        ...
        <div className="flex flex-col items-center justify-center py-20 px-4 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-dashed border-gray-300 dark:border-gray-700">
          <div className="w-20 h-20 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-6">
            <Search className="w-8 h-8 text-gray-400" />
          </div>
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Game tidak ditemukan</h3>
          <p className="text-gray-500 text-center max-w-md">
            Maaf, kami tidak menemukan game yang sesuai dengan filter atau kata kunci pencarian Anda.
          </p>
          <button 
            onClick={() => { setSearchQuery(""); setActiveFilter("ALL"); }}
            className="mt-6 px-6 py-2.5 bg-[#5000ef] hover:bg-[#4000c0] text-white font-bold rounded-xl transition"
          >
            Reset Filter
          </button>
        </div>
