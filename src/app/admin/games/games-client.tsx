...
export default function GamesClient({ initialGames }: { initialGames: Game[] }) {
  ...
  const filteredData = useMemo(() => {
    ...
    return filtered;
  }, [initialGames, searchQuery, sortConfig]);

  const handleDelete = (id: string) => {
    setConfirmDialog({
      isOpen: true,
      title: "Hapus Game",
      message: "Apakah Anda yakin ingin menghapus game ini? Tindakan ini tidak dapat dibatalkan.",
      onConfirm: async () => {
  ...
  const handleExportCSV = () => {
    const headers = ["ID", "NAMA GAME", "DESKRIPSI", "KONSOL", "MULTIPLAYER"];
    const csvContent = [
      headers.join(","),
      ...filteredData.map(item => [
        ...
        `"${item.name}"`,
        `"${item.description || ""}"`,
        `"${item.supported_consoles?.join(" / ") || ""}"`,
        item.is_multiplayer ? "Ya" : "Tidak"
      ].join(","))
    ].join("\n");
    ...
    link.setAttribute("download", `data_game_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const totalGames = initialGames.length;
  const multiplayerCount = initialGames.filter(g => g.is_multiplayer).length;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Pantau dan kelola seluruh koleksi game.
          </p>
        </div>
        <Link href="/admin/games/new" className="flex items-center gap-2 bg-gradient-to-r from-[#5000ef] to-[#00c3cb] text-white px-5 py-2.5 rounded-xl font-bold hover:opacity-90 transition">
      ...
      <div className={`grid grid-cols-2 lg:grid-cols-4 gap-4 ${showStats ? 'grid' : 'hidden lg:grid'}`}>
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col justify-center">
          <span className="text-sm font-medium text-gray-500 flex items-center gap-1.5">
            <Gamepad2 className="w-4 h-4" /> Total Game
          </span>
          <span className="text-3xl font-bold mt-1 text-gray-900 dark:text-white">{totalGames}</span>
        </div>
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col justify-center">
          <span className="text-sm font-medium text-purple-500 flex items-center gap-1.5">
            <Gamepad2 className="w-4 h-4" /> Multiplayer
          </span>
          <span className="text-3xl font-bold mt-1 text-gray-900 dark:text-white">{multiplayerCount}</span>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900/80 border-b border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 font-semibold select-none">
              <tr>
                ...
                <th className="px-6 py-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition select-none group" onClick={() => requestSort('is_multiplayer')}>
                  <div className="flex items-center">FITUR {getSortIcon('is_multiplayer')}</div>
                </th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800 relative">
              {paginatedData.length > 0 ? (
                paginatedData.map((game) => (
                  <tr key={game.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-900/30 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-16 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden flex items-center justify-center shrink-0 border border-gray-200 dark:border-gray-700">
                          {game.image_url ? (
                        ...
                        <div>
                          ...
                          <p className="text-xs font-medium text-gray-500 mt-0.5 truncate max-w-[200px]">{game.description || "-"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-bold">
                      <div className="flex flex-wrap gap-1">
                        {game.supported_consoles?.map((c: string) => (
                          <span key={c} className="font-mono font-bold text-[#5000ef] dark:text-[#00c3cb] text-xs px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700">
                            {c}
                          </span>
                        ))}
                      </div>
                    ...
                    <td className="px-6 py-4">
                      {game.is_multiplayer && (
                        <span className="inline-flex items-center gap-1 px-3 py-1 text-[11px] font-bold rounded-full uppercase tracking-wider bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 border border-purple-200 dark:border-purple-800">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2 relative">
                        {processingId === game.id ? (
                          ...
                          <>
                            <button
                              onClick={() => setOpenActionId(openActionId === game.id ? null : game.id)}
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <Gamepad2 className="w-12 h-12 text-gray-300 dark:text-gray-600 mb-4" />
                      <p className="text-gray-900 dark:text-white font-bold text-lg">Tidak ada game</p>
