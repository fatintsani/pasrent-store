...
import { updateGame } from "@/app/actions/admin/games";
...
export default function EditGameForm({ game }: { game: any }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    let final_cover_url = formData.get("cover_url") as string;
    ...
    setUploadMessage("Menyimpan perubahan...");
    ...
    if (res.success) {
      ...
      setErrorMsg(res.error || "Gagal menyimpan perubahan game.");
      setLoading(false);
      setUploadStatus("error");
    }
  }

  const supportedConsoles = game.supported_consoles || [];
  ...
  return (
    <div className="p-4 md:p-8 w-full mx-auto space-y-6">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight flex items-center gap-3">Edit Game</h1>
          <p className="text-muted-foreground mt-1">Perbarui detail game di katalog rental.</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-6 md:p-8 space-y-6 relative overflow-hidden">
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {game.image_url && (
              <div className="mb-3">
                <p className="text-xs text-gray-500 mb-2">Cover saat ini:</p>
                <img src={game.image_url} alt="Current cover" className="w-24 h-auto rounded-lg border border-gray-200 dark:border-gray-700" />
              </div>
            )}
            ...
            />
            <p className="text-xs text-gray-500">Opsional. Biarkan kosong jika tidak ingin mengubah gambar kover game.</p>
          </div>
                ...
                {loadingTypes ? (
                  ...
                  consoleTypes.map(ct => (
                    <label key={ct.id} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" name="supported_consoles" value={ct.code} defaultChecked={supportedConsoles.includes(ct.code)} className="w-4 h-4 text-[#5000ef] rounded border-gray-300" />
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" name="is_multiplayer" value="true" defaultChecked={game.is_multiplayer} className="w-4 h-4 text-[#5000ef] rounded border-gray-300" />
